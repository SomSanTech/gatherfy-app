...
const CustomBottomSheet = forwardRef<Ref, Props>((props, ref) => {
  ...
  const clearFilters = async () => {
    ...
    props.handleTagPress("Clear");
    props.sorting(""); // บอกฝั่ง backend ว่าไม่จัดเรียง
    setSortingValue(""); // << รีเซ็ต dropdown กลับ default
    props.setIsSearched(false);

    setIsCleared(true); // <<< บอกว่าเคลียร์เสร็จแล้ว
  };
  ...
  useEffect(() => {
    if (isCleared) {
      ...
      setIsCleared(false); // รีเซ็ตสถานะหลังจากเคลียร์เสร็จ
    }
  }, [isCleared]);

  return (
    <GestureHandlerRootView style={styles.bottomModalContainer}>
      {props.isSearched && props.isLoading ? (
        <Loader />
      ) : (
      ...
      >
        ...
        >
          <View className="pb-5">
            ...
            <View className="mb-10 flex-row w-full justify-around items-center">
              <CustomButton
                title="Clear All"
                containerStyles={styles.clearBtn}
